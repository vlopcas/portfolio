...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><header className="page-header case-page-header"><div className="container"><Link className="back-link" href="/projects"><DirectionIcon/> Todos os projetos</Link><div className="case-header"><div><span className={`status status-${p.status}`}>{statusLabels[p.status]}</span><span className="project-area">{p.area}</span><h1 className="page-title">{p.title}</h1><p className="lead">{p.summary}</p><div className="tags">{p.tags.map(t => <span className="tag" key={t}>{t}</span>)}</div></div></div></div></header>
