...
const areas = [
  ...
  { title: "Data Engineering", text: "Pipelines, contratos e camadas de dados para ingestão, processamento e análise.", icon: DataIcon },
  { title: "Data Science", text: "Modelagem de dados com análises preditivas e prescritivas.", icon: ScienceIcon },
...
export default function Home() {
  return <>
    ...
    <section className="container section"><div className="section-head"><div><span className="eyebrow">Áreas de atuação</span><h2>Atuação multidisciplinar.</h2></div></div><div className="grid areas">{areas.map(({title,text,icon:Icon})=><article className="area" key={title}><div className="area-icon"><Icon /></div><h3>{title}</h3><p>{text}</p></article>)}</div></section>
    <section className="container section about-slice home-about"><div><span className="eyebrow">Sobre</span><h2>Minha trajetória entre ciência e tecnologia.</h2></div><div><p>Comecei investigando proteínas, vacinas e dados biológicos. Hoje, aplico essa mesma curiosidade a problemas de dados, software e inteligência artificial, entendendo o contexto antes de escolher a tecnologia.</p><Link className="journey-link" href="/about"><span className="journey-icon"><JourneyIcon /></span><span><strong>Conheça minha trajetória</strong><small>Da Biotecnologia aos sistemas de dados</small></span><DirectionIcon /></Link></div></section>
